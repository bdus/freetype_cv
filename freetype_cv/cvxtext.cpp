...
int CvxText::putText(cv::Mat &frame, const char *text, CvPoint pos, CvScalar color, double wordScale)
{
     FT_Set_Pixel_Sizes(m_face, wordScale, 0);
     return putText(frame,text, pos, color);
}
int CvxText::putText(cv::Mat & frame, const char * ctext, CvPoint pos, CvScalar color)
{
    QString str = QObject::tr(ctext);
    QByteArray ba = str.toLocal8Bit();
    char * text = ba.data();
    //return putText(&IplImage(frame), ba.data(), pos, color);
    ...
    for (i = 0; text[i] != '\0'; ++i)
    {
            ...
            //putWChar(&IplImage(frame), wc, pos, color);
...
//                        p[j+2] = p[j+2] * ( 1- rate ) + color.val[2];
                        ...
                        frame.at<cv::Vec3b>(r,c)[0] = frame.at<cv::Vec3b>(r,c)[0] * rate + color.val[0] * (1-rate);
                        frame.at<cv::Vec3b>(r,c)[1] = frame.at<cv::Vec3b>(r,c)[1] * rate + color.val[1] * (1-rate);
                        frame.at<cv::Vec3b>(r,c)[2] = frame.at<cv::Vec3b>(r,c)[2] * rate + color.val[2] * (1-rate);
...
//{
//    return putText(&IplImage(img),text,pos,color);
//}
